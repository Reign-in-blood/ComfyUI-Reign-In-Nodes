...
class RIN_ImageSizePicker:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "qwen_resolution": (s.MODEL_RESOLUTIONS["Qwen"], {
                    "default": s.MODEL_RESOLUTIONS["Qwen"][0],
                }),
    ...
    def RIN_ImageSizePicker(self, model_type: str, qwen_resolution: str, sdxl_resolution: str, flux_resolution: str,
             batch_size: int, width_override: int = 0, height_override: int = 0) -> tuple:
        
        batch_size = max(1, batch_size)
        
        resolution_map = {
            "Qwen": qwen_resolution,
        ...
        if model_type != "Qwen" and not (width_override > 0 or height_override > 0):
            width = (width // 64) * 64
            height = (height // 64) * 64
